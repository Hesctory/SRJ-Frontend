...
export const EnrollmentSelector = ({ studentId, onSelect }: EnrollmentSelectorProps) => {
    const { data: enrollments, isPending: enrollmentsLoading } = useGetList<Enrollment>(
        "enrollments",
        {
            filter: { studentId },
        ...
        <Box display="flex" flexDirection="column" gap={2}>
            ...
            <Box display="flex" flexWrap="wrap" gap={2}>
                {sorted.map((enrollment) => {
                    const yearLabel = schoolYearMap.get(enrollment.yearId) ?? `Año ${enrollment.yearId}`;
                    return (
                        <Button
                            key={enrollment.id}
                            variant="outlined"
