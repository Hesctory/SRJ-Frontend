...
export const EnrollmentPaymentsEdit = () => {
  ...
  const { data: paymentRecords, isPending: paymentLoading } =
    useGetList<EnrollmentPayment>(
      "enrollment-payments",
      {
  ...
  const selectedTuition =
    tuitions.find((t) => t.monthId === selectedMonthId) ?? null;
            ...
            <Grid container spacing={2}>
              {tuitions.map((tuition) => {
                const isPaid =
                  tuition.status === "paid" ||
                  tuition.amountPaid +
                    (localFees[tuition.monthId]?.reduce((s, f) => s + f, 0) ??
                      0) >=
                    tuition.tuitionCost;
                return (
          ...
          {selectedMonthId !== null && selectedTuition && (
            <Box width="50%" display="flex" flexDirection="column" gap={2}>
              ...
              <Box>
                ...
                {selectedTuition.paymentIds.length === 0 &&
                extraFees.length === 0 ? (
                  <Typography variant="body2" color="text.secondary">
